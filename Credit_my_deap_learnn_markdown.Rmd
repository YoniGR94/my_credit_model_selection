---
title: "Different Technics to Analyze Credit Balance"
...
libraries('tensorflow','keras3') #Neural Network
#install_tensorflow() #install_keras() #install_minicomda
libraries('Matrix','scales') #data tools
#visual
libraries('hrbrthemes','viridis','ggridges')
...
For that purpose, I started to model this data using some common relevant models. in addition, I will compare the result of the models in order to choose the better ones.
...
Here I create logistic prediction of Balance=0 in order to use in as another variable that I might consider using.
...
Now, what is our best predictors, if I filter our variables and prevent over fitting?
...
The chosen model is $$Balance ~ Rating + Income + Student + Limit + Cards$$ and I get sd of 80.94
...
But this time I use shrinkage method in order to reduce variance & over fitting. so our minimizing function define as
...
This time, I need to set our hipper parameter, 
$\lambda$
...
```{r Keras result, cache=T, message=F, warning=F, error=F}
mod_Credit
...
I use the same seed to test all methods, so now I can compare the error of each data.

* Each time this script were running, I got different result, due to randomness of $testid$ , and of the deep learning models
